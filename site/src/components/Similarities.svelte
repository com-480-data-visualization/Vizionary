	...
	function findSimilarSports(targetSportName, allSimilarities) {
		...
		for (const [sportKey, sportData] of Object.entries(allSimilarities)) {
			if (sportKey !== targetSportName) {
				const similarity = calculateCosineSimilarity(targetSport, sportData);
				...
				<tbody>
					{#each similarSports as sport}
						...
						>
							...
							<td class="similarity">{(sport.similarity * 100).toFixed(1)}%</td>
							<td class="correlation age">{sport.age.toFixed(4)}</td>
							<td class="correlation height">{sport.height.toFixed(4)}</td>
							<td class="correlation weight">{sport.weight.toFixed(4)}</td>
						</tr>
					{/each}
				</tbody>
			</table>
